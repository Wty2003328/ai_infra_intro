...
> **Layer:** L8. **Prerequisites:** [Attention_Mechanisms](../L6_Algorithms_and_Models/02_Attention_Mechanisms.md), [KV_Cache](01_KV_Cache.md), [Modern_KV_Compression](02_Modern_KV_Compression.md). **Hands off to:** [Production_Architecture](15_Production_Architecture.md).
...
Chunked prefill (see [Batching_and_Scheduling](03_Batching_and_Scheduling.md) for the full scheduling treatment) breaks the prompt into chunks of $C$ tokens (typically 512--4096). Each chunk is processed as a separate prefill operation, accumulating the KV cache incrementally.
...
2. **KV cache fragmentation.** As chunks are processed, the KV cache grows incrementally. PagedAttention (see [KV_Cache](01_KV_Cache.md)) allocates new pages dynamically, but at 1M tokens the page table itself becomes large (4K--16K pages).
...
At 1M context with a 70B model, a single 8-GPU node serves at most 7 concurrent requests. At 10M context, it serves at most 1 request across the entire node. This is why KV compression (from [Modern_KV_Compression](02_Modern_KV_Compression.md)) is essential for long-context serving.
...
| Direction | Link |
|-----------|------|
| Prerequisite | [Attention_Mechanisms](../L6_Algorithms_and_Models/02_Attention_Mechanisms.md) |
| Prerequisite | [KV_Cache](01_KV_Cache.md) |
| Prerequisite | [Modern_KV_Compression](02_Modern_KV_Compression.md) |
| Prerequisite | [Batching_and_Scheduling](03_Batching_and_Scheduling.md) |
| Next | [Production_Architecture](15_Production_Architecture.md) |

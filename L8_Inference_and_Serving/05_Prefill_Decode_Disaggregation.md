...
> **Layer:** L8.
> **Prerequisites:** [Batching_and_Scheduling](03_Batching_and_Scheduling.md), [Rack_Scale_Design](../L4_Systems_and_Interconnects/02_Rack_Scale_Design.md).
> **Hands off to:** [Disaggregated_Serving_2025](10_Disaggregated_Serving_2025.md), [Production_Architecture](15_Production_Architecture.md).
...
Recall from [Memory_Hierarchy_and_Roofline](../L3_Microarchitecture/03_Memory_Hierarchy_and_Roofline.md) the roofline model: achievable performance $P = \min(\pi,\; I \cdot \beta)$, where $\pi$ is peak FLOPS, $\beta$ is peak memory bandwidth, and $I$ is arithmetic intensity (FLOP/byte).
...
When both phases share one GPU pool under continuous batching (see [Batching_and_Scheduling](03_Batching_and_Scheduling.md) section 4), a mixed step containing $K$ prefill tokens and $B$ ongoing decode sequences takes:
...
Speculative decoding (see [Speculative_Decoding](04_Speculative_Decoding.md)) runs a cheap draft model to predict K future tokens, then verifies in a single decode step. In a disaggregated system:
...
For mixture-of-experts models (see [Modern_MoE](../L6_Algorithms_and_Models/03_Modern_MoE.md)), disaggregation interacts with expert parallelism (EP):

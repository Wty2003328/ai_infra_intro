...
A: FA-v1: tiled forward with online softmax, first to avoid materializing the $N \times N$ matrix. FA-v2: split-Q work partition (parallelize over Q blocks instead of K/V blocks for better GPU utilization) + better causal masking that skips empty blocks entirely -- 2x over v1. FA-v3: Hopper-specific optimizations including warp specialization (producer/consumer warps), TMA for async tile loads, FP8 via wgmma instructions, achieving ~75% of peak FP16 TFLOPS on H100. See [FlashAttention_Deep_Dive](../L5_Kernels_and_Programming/05_FlashAttention_Deep_Dive.md).
...
- Day 4-7: Practice 4-6 system designs (see [System_Design_Interview](10_System_Design_Interview.md)). Time yourself to 45 minutes each.
- Day 8-10: Coding patterns (see [Coding_Patterns](11_Coding_Patterns.md)). Write kernels on a blank editor without references.
...
- [System_Design_Interview](10_System_Design_Interview.md) -- design framework + worked examples.
- [Coding_Patterns](11_Coding_Patterns.md) -- the algorithmic / kernel-level coding side.

---

**Next:** [Coding_Patterns](11_Coding_Patterns.md).

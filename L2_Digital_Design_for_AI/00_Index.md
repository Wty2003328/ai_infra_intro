...
2. **FP unit design** — from CLA/CRA through implementation-ready unpack/classify, internal width and binary-point derivation, GRS rounding, near/far add/subtract, segmented significand multipliers, fused carry-save FMA, tensor dot-product accumulator choices, divider/root FSMDs, conversions/SFUs, execution-port control, physical implementation, and verification.
...
4. **AI-specific RTL patterns** — pipelining and timing closure ($f_{\max}$ derivation, FO4 budget); CDC and mesochronous interfaces; conventional and tensor DMA command/AGU/tag/buffer/completion pipelines; async-proxy/barrier handoff; on-die NoC plus inter-chip endpoint/link/PHY/switch/order/coherence design; clock/power gating; verification reality.
...
2. [FP_Unit_Design](02_FP_Unit_Design.md) — complete scalar/tensor arithmetic from integer building blocks, including exact registered state, shift/tail/rounding hardware, multi-format segmentation, FMA/dot accumulation choices, iterative FSMD control, execution-port integration, physical signoff, and bit-exact/formal verification. The compute atom.
...
4. [Digital_Design_For_AI](04_Digital_Design_For_AI.md) — the integration layer: pipelining, CDC, descriptor/tensor DMA and TMA-style async copies, GPU NoC, and the full inter-chip endpoint→link→PHY→switch→remote-memory path with UCIe/PCIe/CXL/scale-up semantic boundaries. **Read last** because it relies on the prior three.
...
| A D2D clock contract determines sampling training, lane deskew, elastic buffering, and CDC | Cross-die latency must be derived from the implemented PHY/buffer/protocol/remote-target path, not a universal cycle penalty |
...
4. **[Digital_Design_For_AI](04_Digital_Design_For_AI.md)** — the integration glue (pipelining, CDC, DMA/TMA, proxy synchronization, NoC, and inter-chip fabrics).

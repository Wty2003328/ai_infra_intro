...
> **Position in the stack:** sits between hand-written kernels ([Triton_and_Kernels](04_Triton_and_Kernels.md), [CUDA_Optimization](02_CUDA_Optimization.md)) and the frameworks that run models ([vLLM_Internals](../L8_Inference_and_Serving/09_vLLM_Internals.md), [Distributed_Training](../L7_Training_Stack/03_Distributed_Training.md)). Hand-written kernels set the performance ceiling; compilers decide how close the other 95% of the model gets to it.
...
The fusion payoff is pure roofline math (see [Memory_Hierarchy_and_Roofline](../L3_Microarchitecture/03_Memory_Hierarchy_and_Roofline.md)). For a chain of $k$ elementwise ops on a tensor of $N$ bytes:
...
- **Static addresses** — tensors must live at the same pointers across replays. → Persistent buffer pools; KV cache is pre-allocated paged memory anyway ([KV_Cache](../L8_Inference_and_Serving/01_KV_Cache.md)).
...
- XLA runs aggressive fusion (producer-consumer, multi-output), layout assignment (picks physical tile layouts per op for the MXU's 128×128 systolic array — see [Google_TPU](../L3_Microarchitecture/06_Google_TPU.md)), and buffer assignment (static memory plan — no allocator at runtime).
...
1. **Algorithmic rewrites.** No compiler turns naive attention into FlashAttention — that's a *math* transformation (online softmax, see [FlashAttention_Deep_Dive](05_FlashAttention_Deep_Dive.md)) changing the memory complexity class, not a fusion. Compilers fuse within the algorithm you wrote.
...
- Upstream: [Triton_and_Kernels](04_Triton_and_Kernels.md) (the codegen target), [CUDA_Optimization](02_CUDA_Optimization.md) (what good output looks like), [Google_TPU](../L3_Microarchitecture/06_Google_TPU.md) (XLA's hardware).
- Downstream: [vLLM_Internals](../L8_Inference_and_Serving/09_vLLM_Internals.md) (piecewise graphs in production), [Batching_and_Scheduling](../L8_Inference_and_Serving/03_Batching_and_Scheduling.md) (bucketing), [Training_Optimization](../L7_Training_Stack/04_Training_Optimization.md) (compile in training loops).

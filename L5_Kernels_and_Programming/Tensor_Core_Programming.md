...
CUDA-graph basics (capture, instantiate, replay) are covered in [CUDA_Optimization](CUDA_Optimization.md) §7.3. In inference serving, the decode phase processes variable batch sizes. Re-capturing a CUDA graph for each new batch size is expensive. Hopper-era drivers support **graph node update**: instantiate the graph once, then update kernel node parameters before each replay.
...
- [CUDA_Programming](CUDA_Programming.md) — thread hierarchy, memory model, streams, and the tiled-matmul capstone this page accelerates.
- [CUDA_Optimization](CUDA_Optimization.md) — coalescing, tiling, bank conflicts, occupancy: the ladder rungs below tensor-core work.
- [GPU_Architecture](../L3_Microarchitecture/GPU_Architecture.md) — SM anatomy, tensor core generations, warp scheduling.
- [Blackwell_Architecture](../L3_Microarchitecture/Blackwell_Architecture.md) — TMEM, 5th-generation tensor cores, and the hardware context for SM100 kernels.

**Feeds into (higher-layer pages that use this):**
- [FlashAttention_Deep_Dive](FlashAttention_Deep_Dive.md) — FA-2/FA-3 are warp-specialized TMA + wgmma pipelines end to end.
- [Cutting_Edge_Kernels](Cutting_Edge_Kernels.md) — CUTLASS 3.x collective builders package the patterns on this page as composable C++ templates.
- [Triton_and_Kernels](Triton_and_Kernels.md) — what the Triton compiler automates (and what it does not yet) of this pipeline.

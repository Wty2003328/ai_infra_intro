...
See [Parallelism_Strategies](Parallelism_Strategies.md) for expert parallelism details.
...
The cache layout matters for both correctness (kv positions must be in the right order) and performance (contiguous access for coalesced loads). See [KV_Cache](KV_Cache.md) for the deep dive on PagedAttention's layout.

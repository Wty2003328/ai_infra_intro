...
**Prerequisites**: [Prefill_Decode_Disaggregation](../Part_6_Inference_Engines_and_Serving/Prefill_Decode_Disaggregation.md), [Batching_and_Scheduling](../Part_6_Inference_Engines_and_Serving/Batching_and_Scheduling.md), [Networking_and_Interconnect](../Part_3_Hardware_Systems_and_Interconnects/Networking_and_Interconnect.md), [KV_Cache](../Part_6_Inference_Engines_and_Serving/KV_Cache.md).
...
### 4.5 Pool Sizing Math and The Reasoning Shift

For request rate $\lambda$, average prompt length $S_p$, output length $S_d$:

**Prefill pool**:
$GPUs_p = \frac{\lambda \cdot 2 \cdot \text{params} \cdot S_p}{\eta_p \cdot \text{peak\_FLOPs}}$

**Decode pool**:
$GPUs_d = \frac{\lambda \cdot S_d \cdot (\text{weights} + \text{KV}(S_p + S_d/2) \text{ bytes})}{\eta_d \cdot \text{HBM\_BW}}$

Scale each to its own SLO. Adjust ratio dynamically as workload shifts.

**The Reasoning Shift**:
Historically (2023-2024), standard chat models had $S_p \approx 1000$ and $S_d \approx 200$. The Prefill:Decode compute ratio was roughly 1:1 or 1:2 depending on batching.
With the rise of Reasoning models (OpenAI o1, DeepSeek-R1), the output length explodes ($S_d = 10,000$ to $100,000+$) due to latent Chain-of-Thought generation. 
- The decode phase now dominates cluster time. A single request occupies a decode slot for minutes.
- To maintain system throughput, capacity planners in 2026 deploy clusters with **Prefill : Decode GPU ratios of 1:10 to 1:20**.
- The sheer volume of decode traffic also means the KV Transfer network (NIXL) becomes less of a bottleneck, as the transfer cost is amortized over tens of thousands of decode steps. If the KV cache is in FP4, the transfer payload is reduced by 4x, further trivializing the network hop.
...
**Next:** [Modern_Post_Training](../Part_7_Distributed_Systems_and_Scale/Modern_Post_Training.md), [Multimodal_Inference](Multimodal_Inference.md).
**See also:** [Prefill_Decode_Disaggregation](../Part_6_Inference_Engines_and_Serving/Prefill_Decode_Disaggregation.md), [Modern_KV_Compression](../Part_6_Inference_Engines_and_Serving/Modern_KV_Compression.md), [Inference_Frameworks](Inference_Frameworks.md).

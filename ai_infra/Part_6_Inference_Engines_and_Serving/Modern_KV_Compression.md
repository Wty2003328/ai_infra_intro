...
**Prerequisites**: [KV_Cache](KV_Cache.md), [Attention_Mechanisms](../Part_5_Algorithms_and_Quantization/Attention_Mechanisms.md), [Long_Context_Engineering](Long_Context_Engineering.md), [Quantization](../Part_5_Algorithms_and_Quantization/Quantization.md).
...
Microscaling FP4: blocks of 16 or 32 elements share an exponent. Native tensor-core support on Blackwell. KV at 4 bits with hardware-accelerated dequantization.
- **Compression**: ~4× vs FP16, 2× vs FP8.
- **Hardware Mechanics**: During the FlashAttention forward pass, the MMA (Matrix Multiply-Accumulate) unit fetches the packed FP4 KV blocks from TMEM. The shared 8-bit exponent is applied natively in the datapath before the dot product with the FP8/FP16 Query vector. This entirely hides the dequantization cost, making FP4 KV strictly limited by HBM read bandwidth.
- **Why it's critical for Reasoning**: Models like DeepSeek-R1 generate 30K+ tokens of "thinking" before producing an answer. This massive generation phase requires a massive KV cache. Without FP4 KV compression, a single reasoning request could consume 100+ GB of HBM, limiting concurrency to near zero. FP4 KV expands the concurrent reasoning batch size by 4x over FP16, shifting the bottleneck back to compute.
...
- **Disaggregated KV transfer**: NIXL moves KV between prefill and decode pools or between active and standby replicas. See [Networking_and_Interconnect](../Part_3_Hardware_Systems_and_Interconnects/Networking_and_Interconnect.md).
...
**Next:** [State_Space_Models_and_Hybrids](../Part_5_Algorithms_and_Quantization/State_Space_Models_and_Hybrids.md), [Disaggregated_Serving_2025](../Part_6_Inference_Engines_and_Serving/Disaggregated_Serving_2025.md).
**See also:** [KV_Cache](KV_Cache.md), [Long_Context_Engineering](Long_Context_Engineering.md), [Quantization](../Part_5_Algorithms_and_Quantization/Quantization.md).

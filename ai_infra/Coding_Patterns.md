...
**Prerequisites**: [CUDA_Programming](CUDA_Programming.md), [CUDA_Optimization](CUDA_Optimization.md), [Triton_and_Kernels](Triton_and_Kernels.md), [Batching_and_Scheduling](Batching_and_Scheduling.md).
...
Key: subtract max **before** exp for numerical stability. Online (block-wise) softmax needed when N exceeds BLOCK_N — see [Attention_Mechanisms §3](Attention_Mechanisms.md).
...
Whiteboard expectation: you should be able to write this skeleton — outer loop over Q blocks, inner over K/V, online softmax accumulator — in 20-30 minutes. See [FlashAttention_Deep_Dive §12](FlashAttention_Deep_Dive.md) for the full version.
...
**See also:** [CUDA_Programming](CUDA_Programming.md), [CUDA_Optimization](CUDA_Optimization.md), [Triton_and_Kernels](Triton_and_Kernels.md), [System_Design_Interview](System_Design_Interview.md).

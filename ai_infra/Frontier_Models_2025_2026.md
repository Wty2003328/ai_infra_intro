...
**Prerequisites**: [Transformer_Internals](Transformer_Internals.md), [Attention_Mechanisms](Attention_Mechanisms.md), [Modern_MoE](Modern_MoE.md), [Long_Context_Engineering](Long_Context_Engineering.md).
...
The RL algorithm. Difference from PPO:
- No critic model.
- Instead, sample **K completions per prompt** (e.g., K=16), compute reward for each, normalize within the group.
- Advantage = `(reward - group_mean) / group_std`.

Memory savings: only 2 model copies (policy + reference, both 671B) instead of 4 (policy, ref, reward, critic). Critical for RL on huge models.
...
**See also:** [Attention_Mechanisms](Attention_Mechanisms.md), [Long_Context_Engineering](Long_Context_Engineering.md), [Modern_Quantization_Frontier](Modern_Quantization_Frontier.md).

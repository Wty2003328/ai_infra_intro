...
flowchart TD
    ...
    J["9. Vector types & ILP<br/>(float4, uint4, instruction-level<br/>parallelism)"] --> K
    K["10. Launch overhead<br/>(fusion, CUDA graphs,<br/>persistent kernels)"]
    ...
    style K fill:#f1f5f9,stroke:#475569,color:#000
```
...
Global memory on Hopper delivers ~3 TB/s (HBM3). The SM's compute units consume data at ~20-100 TB/s internally. The ratio is 7-33x. Tiling copies a reusable data block from HBM into SMEM (19 TB/s) or registers (register file bandwidth ~64 TB/s), amortizing the slow HBM transfer across many arithmetic operations.
...
| Level | Instruction | Threads involved | Tile shape (FP16/BF16) | Architecture |
|-------|------------|-----------------|----------------------|-------------|
| WMMA | `wmma::mma_sync` | 1 warp (32) | $16 \times 16 \times 16$ | Volta+ |
| WGMMA | `wgmma.mma_async` | 1 warp group (128) | $64 \times N \times 16$ | Hopper+ |
...
## 10. Vector types and instruction-level parallelism

### 10.1 Vector loads and stores
...
### 10.2 ILP — independent instruction chains
...
## 12. Profiling with Nsight Compute and Nsight Systems

### 12.1 Nsight Systems — system-level timeline
...
### 12.2 Nsight Compute — kernel-level roofline
...
### 12.3 Iterative optimization workflow
...
## 13. Case study — parallel reduction
...
### 13.2 Optimized reduction — sequential addressing
...
### 13.3 Performance progression

| Version | Bandwidth (GB/s) on A100 | % of peak | Bottleneck |
|---------|--------------------------|-----------|-----------|
| Naive (global atomics) | 120 | 4% | Contention |
| SMEM, sequential addressing | 1 200 | 40% | Bank conflicts |
| + warp shuffle | 1 650 | 55% | Launch overhead |
| + vector loads (float4) | 1 950 | 65% | Memory coalescing limit |
| + multi-stage (kernel fusion) | 2 400 | 80% | Approaching roofline |

---

## 14. End-to-end optimization flow
...
flowchart TD
    ...
    ASYNC --> PROF["Profile with Nsight Compute"]
...
| SMEM bandwidth per SM | ~19 TB/s | Shared memory read/write |
...
| `wgmma` throughput per SM (Hopper, FP16) | ~128 TFLOPS | 4th gen tensor core |

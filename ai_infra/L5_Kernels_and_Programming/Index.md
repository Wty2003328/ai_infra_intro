...
1. [CUDA_Programming](CUDA_Programming.md) — the CUDA programming model from scratch. Thread hierarchy, memory spaces, synchronization, streams, complete matmul example.
2. [CUDA_Optimization](CUDA_Optimization.md) — the optimization hierarchy: coalescing, shared memory tiling, bank conflicts, occupancy, tensor cores, async copy, TMA, warp specialization, profiling with Nsight.

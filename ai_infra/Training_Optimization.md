...
**Prerequisites**: [Distributed_Training](Distributed_Training.md), [CUDA_Optimization](CUDA_Optimization.md), [Memory_Hierarchy_and_Roofline](Memory_Hierarchy_and_Roofline.md).
...
Mandatory. Drops attention from compute-and-memory-bound to compute-bound, saves O(N²) memory. See [FlashAttention_Deep_Dive](FlashAttention_Deep_Dive.md).
...
A: (1) Fuse independent ops (norm + linear + bias). (2) Use FlashAttention for the attention. (3) Tile sizes that fit shared memory but maximize tensor core utilization. (4) Async pipeline (TMA on Hopper). (5) Verify register pressure isn't dropping occupancy. (6) Compare against CUTLASS reference. See [CUDA_Optimization](CUDA_Optimization.md).
...
**See also:** [CUDA_Optimization](CUDA_Optimization.md), [Distributed_Training](Distributed_Training.md), [Quantization](Quantization.md), [Observability_and_Debugging](Observability_and_Debugging.md).

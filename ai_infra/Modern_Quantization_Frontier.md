...
**Prerequisites**: [Quantization](Quantization.md), [Blackwell_Architecture](Blackwell_Architecture.md), [GPU_Architecture](GPU_Architecture.md).
...
- **Floating vs integer**: floating handles dynamic range better; integer is cheaper kernels.
- **Block size**: smaller blocks = more scales = better accuracy; larger blocks = less metadata overhead.
...
KIVI (covered in [Modern_KV_Compression](Modern_KV_Compression.md)): K per-channel, V per-token, 2 bits each with asymmetric scales. ~1pp MMLU drop. Niche capacity-constrained deployments.
...
**Next:** [Disaggregated_Serving_2025](Disaggregated_Serving_2025.md), [Modern_Post_Training](Modern_Post_Training.md).
**See also:** [Quantization](Quantization.md), [Blackwell_Architecture](Blackwell_Architecture.md), [Modern_KV_Compression](Modern_KV_Compression.md).
